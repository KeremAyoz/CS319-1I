...
public class Tournament implements Serializable {
	...
	/*
	 * public void updateMyGroupMatchInfo( ArrayList<Action> actions , Team home
	 * , Team away ) {
	 * 
	 * int idHome = -1 , idAway = -1;
	 * 
	 * for( int i = 0 ; i < TEAMS_PER_GROUP ; i++ ) if( home ==
	 * groups[myGroupId].getTeam(i) ) idHome = i;
	 * 
	 * for( int i = 0 ; i < TEAMS_PER_GROUP ; i++ ) if( away ==
	 * groups[myGroupId].getTeam(i) ) idAway = i;
	 * 
	 * int goalHome = 0 , goalAway = 0; int totalAction = actions.size();
	 * 
	 * for( int i = 0 ; i < totalAction ; i++ ) { if( actions.get(i).getClass()
	 * == Goal.class ) { Goal goal = (Goal) actions.get(i); boolean foundHome =
	 * false; for( int j = 0 ; j < home.getPlayers().size() && !foundHome ; j++
	 * ) if( home.getPlayers().get(j) == goal.getScored() ) foundHome = true;
	 * if( foundHome ) goalHome++; else goalAway++; goal.getScored().setCntGoal(
	 * goal.getScored().getCntGoal() + 1 ); goal.getAssisted().setCntAssist(
	 * goal.getAssisted().getCntAssist() + 1 ); } else if(
	 * actions.get(i).getClass() == YellowCard.class ) { YellowCard yellowCard =
	 * (YellowCard) actions.get(i); // yellowCard.getPlayer().setCntYellowCard(
	 * yellowCard.getPlayer().getCntYellowCard() + 1 ); } else if(
	 * actions.get(i).getClass() == RedCard.class ) { RedCard redCard =
	 * (RedCard) actions.get(i); // redCard.getPlayer().setCntRedCard(
	 * redCard.getPlayer().getCntRedCard() + 1 ); } else if(
	 * actions.get(i).getClass() == Injury.class ) { Injury injury = (Injury)
	 * actions.get(i); // } }
	 * 
	 * // May be wrong for( int i = 0 ; i < NUMBER_OF_PLAYERS ; i++ ) {
	 * home.getPlayers().get(i).setCntMatch(
	 * home.getPlayers().get(i).getCntMatch() + 1 );
	 * away.getPlayers().get(i).setCntMatch(
	 * away.getPlayers().get(i).getCntMatch() + 1 ); }
	 * 
	 * int idMatch = -1; for( int i = 0 ; i < MATCHES_PER_GROUP && idMatch == -1
	 * ; i++ ) if( groups[myGroupId].getMatch(i).getHome() == home ) if(
	 * groups[myGroupId].getMatch(i).getAway() == away ) idMatch = i;
	 * 
	...
	/*
	 * public void playMyGroupMatch() throws InterruptedException { int
	 * nextMatchId = myGroupMatchIds[lastMatchWeek + 1]; lastMatchWeek++;
	 * lastMatchId = nextMatchId; groups[myGroupId].playMatch(nextMatchId);
	 * goNextDay(); }
	 */

	/*
	 * public boolean isGroupStage() { if( lastMatchWeek <= MID_SEASON_WEEK )
	 * return true; else return false; }
	 */

	/*
	 * public boolean isEliminationStage() { if( lastMatchWeek > MID_SEASON_WEEK
	 * ) return true; else return false; }
	 */
